...
imagesRoutes.get('/', async (req: Request, res: Response) => {
    ...
    if (
        !fs.existsSync(
            path.join(
                __dirname,
                `../assets/originalImages/${req.query.fileName}.jpg`
            )
        )
    ) {
    ...
    if (
        !fs.existsSync(
            path.join(
                __dirname,
                `../assets/editedImages/${req.query.fileName}_${width}x${height}.jpg`
            )
        )
    ) {
        ...
        await resizeImage(imagePath, destination, width, height).catch(
            (err: NodeJS.ErrnoException) => {
                console.log(err.message);
            }
        );
        const image = await getImage(destination).catch(
            (err: NodeJS.ErrnoException) => {
                console.log(err.message);
            }
        );
        ...
        const image: Buffer | void = await getImage(imagePath).catch(
            (err: NodeJS.ErrnoException) => {
                console.log(err.message);
            }
        );
